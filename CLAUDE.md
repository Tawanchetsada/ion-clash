...
A working Next.js 16 codebase. Phases 0–10 are done: tooling + CI, pure chemistry domain, 50 levels of data, save system, game state machine, design-system component library, drag-and-drop interaction system, all application screens/routes, Grade 10 Thai learning content (`/knowledge`, `/how-to-play`, refined 150 hints, in-game solubility rules dialog), research data collection system (Dual Sink, CSV/TSV export with BOM, `/research` E1/E2 calculation & error analysis dashboard, consent flow, and Google Apps Script integration), and comprehensive test/QA suites (14 E2E scenarios across 3 viewports, fault injection, WCAG AA/AAA accessibility audit, viewport responsiveness, route guards). Verified end-to-end (1,658 unit/component tests + 138 Playwright E2E tests across Desktop, iPad, Mobile). Phase 11 (deploy & handoff) is next.
...
| `npm run gen:audio` | Regenerate `public/audio/*.wav` from Kenney sources (downloads them if `scripts/audio-src/` is missing) |
...
**Four separated layers.** `src/domain/` (pure chemistry + game logic, no React), `src/data/` (ions, compounds, 50 levels as typed data), `src/presentation/` (pure functions turning domain data into view models — formulas, aria-labels, tones, level-tile status), `src/components/` (dumb rendering). Components emit events; they never validate chemistry themselves. `IonSlot` accepts a placement, it does not decide whether the placement is chemically right.

The domain boundary is enforced, not merely documented: an ESLint `no-restricted-imports` block scoped to `src/domain/**` bans `react`/`next`, a second block scoped to `src/components/**` bans importing *values* from `src/domain/**` or `src/data/**` (type-only imports are allowed — erased at build, so no runtime coupling), and `src/architecture.test.ts` scans the filesystem to back both up — and to prove `solubility.ts` is the only file in `src/` that ever assigns `phase: "s"`, that nothing outside `src/storage/` touches `localStorage`, and that no color token in `globals.css` has drifted from the spec's Design System table.

**What exists today.** `src/domain/chemistry/` holds `ions` (22 ions per D-02), `solubility` (11 ordered rules — rule order is load-bearing; see its header comment), `formula`, `compounds` (criss-cross), `balance` (brute force 1–12, throws rather than guessing), `reaction` (cross-exchange + D-03 pairing validation), `spectators`, `netIonic` (two-stage reduction). `src/data/` turns a 4-field `LevelSeed` into a fully computed `BuiltLevel` — never hand-write derived chemistry into the seed file. `src/domain/game/` holds the reducer and everything around it: `instances` (D-21 id derivation), `guards` (wraps the chemistry validators; **never re-decides chemistry itself**), `timer`, `scoring`, `hints`, `feedback`, `selectors`, `checkpoint` (GameState ⇄ `LevelCheckpoint`), `gameMachine`. The reducer takes the level as a third argument — `reduce(state, event, level)` — so the game layer never imports `src/data` at runtime. `src/components/{ui,layout,game,levels}/` are the design-system pieces (`Button`, `Dialog`, `AppHeader`, `StepIndicator`, `IonCard`, `EquationView`, `EquationStrip`, `LevelGrid`, …) — see `/dev/components` for every state of every one. `src/audio/` (`AudioEngine`, `AudioProvider`) plays the 5 Kenney CC0 sounds in `public/audio/` (`.wav`, not `.mp3` — Safari/iPad can't play `.ogg` and there's no `ffmpeg` locally; see `scripts/prepare-audio.ts` and `public/audio/CREDITS.md`).

**Game cards size themselves from their container, not from breakpoints.** The step-2 pairing row and the step-4 ionic-equation strip must show the whole equation on one line — a student who can't yet read ionic equations has to see both sides of the arrow at once, so a strip that scrolls half an equation into view defeats the layout. `globals.css` defines `.fit-cards` (`container-type: inline-size`) plus `.fit-cards-track` (step 2, fixed card count) and `.fit-cards-line` (the strip, variable count via a `--fit-units` inline style computed by `EquationStrip`). Both set `--card-size`; `GameCardFace size="fluid"` and `IonSlot size="fluid"` derive **every** dimension and font size from it in `em`, so a card scales as one unit. Don't reintroduce fixed card widths in those two rows, and don't drop `--fit-units` if the card count ever stops being 7. `e2e/viewports.spec.ts` asserts `scrollWidth <= clientWidth` for both regions at ≥768px — page-level overflow tests alone would not catch a regression here, since the old scrolling layout passed them.

**Any top-level page container is a flex item of `<body>`** (`layout.tsx` sets `flex flex-col` on it), so it needs both `w-full` and `min-w-0` or a wide descendant (like the horizontally-scrolling equation strip) will silently force the *entire page* to scroll horizontally instead of scrolling within itself — `overflow-x: auto` alone does not stop a flex item from demanding its min-content width. `PageShell` already carries both classes; don't strip them, and give any other direct child of `<body>` the same treatment.
...
`instanceId` must be **derived deterministically from level data** (D-21) — never `crypto.randomUUID()`, never a render-order counter. Checkpoints persist slot assignments by `instanceId`, so rebuilding level *N* must reproduce byte-identical ids or mid-level resume silently restores nothing. The chemistry domain deliberately does *not* know about instances: it works in `IonTerm` (`ionId` + `count` + `phase`), and instances are layered on above it. `src/domain/game/instances.ts` is the only place ids are minted (`L1:react:a:cat`, `L1:slot:0`, `L1:ci:l:ion:nitrate`); components must call it rather than numbering cards as they render.
